...
// Hollow corporate buzzwords that weaken technical answers. SINGLE SOURCE OF TRUTH:
// the live delivery coach flags these, AND the AI prompts (api/_lib/interview.js) tell the
// model to avoid them — so live feedback and generated answers never contradict each other.
export const JARGON = [
  ...
  'tapestry', 'realm', 'underscore', 'multifaceted'
]
// Comma-joined form for interpolating into LLM prompts ("never use these words: …").
export const BANNED_WORDS = JARGON.join(', ')
...
// Compile each term's regex ONCE per term-list (FILLERS/JARGON/HEDGES are static), cached
// by the array identity — analyze() runs on every keystroke in Solo, so recompiling ~55
// regexes per call was pure waste.
const _termRe = new WeakMap()
function compiledTerms(terms) {
  let c = _termRe.get(terms)
  if (!c) { c = terms.map(t => [t, new RegExp(`(?<![\\w-])${escapeRegExp(t)}(?![\\w-])`, 'g')]); _termRe.set(terms, c) }
  return c
}
...
function countTerms(text, terms) {
  ...
  const items = []
  let count = 0
  for (const [term, re] of compiledTerms(terms)) {   // .match(/g) ignores lastIndex — safe to reuse
