...
// (the `tv` claim) against the stored value — password reset / logout bumps it, which instantly
// invalidates every token issued before that event.
...
export function signToken(userId, tokenVersion = 0) {
  // Access tokens stay short-lived by default; clients call POST /auth/refresh while signed in.
